...
- **English docs, `docs/legacy/` archive, and the paste-a-prompt installer removed — the kit is now Spanish-only and documents a real multi-component setup, not a single prompt.** Deleted: the 12 `*.en.md` translations, `INSTALAR_MEMORIA.md`, the `PROMPT_ULTRA_COMPLETO.{linux,macos}.md` redirect stubs, and `docs/legacy/` (the v1 prompt archive). CI drops the ES/EN parity step (`scripts/check-es-en-parity.mjs` deleted) and the legacy-prompt PowerShell lint (`.github/scripts/extract-and-lint.ps1` deleted). Inbound links across `README.md`, `GETTING_STARTED.md`, `AGENTS.md`, `docs/`, `SECURITY.md`, `CONTRIBUTING.md` and the issue/PR templates were updated; ADRs keep their historical (code-span) references.
...
- **Capítulo v2 → v3:** guía pública **stdio + `obsidian-memoryd` / git manual** por defecto; HTTP y tareas Windows como opciones **definidas por quien instala**. [`docs/migration/v2-to-v3-script-free-kit.md`](./docs/migration/v2-to-v3-script-free-kit.md).
